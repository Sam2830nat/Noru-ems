...
interface AssignEmployeeToDeptModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
...
export default function AssignEmployeeToDeptModal({ isOpen, onClose, onSuccess, department }: AssignEmployeeToDeptModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      // Fetch employees, perhaps only active ones, but limit=100 for a simple dropdown
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative glass-card w-full max-w-md p-6 animate-in fade-in zoom-in duration-200">
        <div className="flex items-start justify-between mb-5 border-b pb-4" style={{ borderColor: 'var(--border-color)' }}>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm" style={{ color: 'var(--text-muted)' }}>To {department.name}</p>
            </div>
          </div>
        ...
        <div className="mt-6 flex justify-end gap-3 pt-4 border-t" style={{ borderColor: 'var(--border-color)' }}>
          ...
          <button type="submit" form="assign-employee-form" disabled={loading} className="btn-primary min-w-[120px] flex justify-center items-center gap-2">
